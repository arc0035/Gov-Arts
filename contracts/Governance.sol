...
contract GovernorAlpha is Owner{
    
    using SafeMath for uint256;

    string public constant name = "Compound Arts";
    ...
    struct Proposal {
        ...
        /// @notice The block at which voting end
        uint endBlock;
    ...
    function _castVote(address voter, uint proposalId, bool support) internal {
        require(state(proposalId) == ProposalState.Active, "castVote: voting is closed");
        uint256 votes = weightMgr.delegated(voter) ;
        require(votes > 0, "You are not delegatee");
        Proposal storage proposal = proposals[proposalId];
